...
from .models import TimeMainTopic, TimeRecord, TimeSubTopic
from .forms import TimeRecordForm
from apps.question_tracking.forms import QuestionRecordForm
import datetime
...
def index(request):
    ...
    progress = TimeMainTopic.objects.filter(user=request.user)
    qprogress = QuestionMainTopic.objects.filter(user=request.user)
    report = []
    
    datas = TimeRecord.objects.filter(topic__main_topic__user=request.user).order_by("date")
    for data in datas:
        if len(report) != 0:
            if data.date == report[-1].date:
                report[-1].hour += data.hour
            else:
                report.append(data)
        else:
            report.append(data)
...
def sub_index(request, pk):
    ...
    progress = TimeMainTopic.objects.filter(user=request.user, id=pk)
    tprogress = TimeMainTopic.objects.filter(user=request.user)
    qprogress = QuestionMainTopic.objects.filter(user=request.user)
    report = []
    
    datas = TimeRecord.objects.filter(topic__main_topic__id=pk, topic__main_topic__user=request.user).order_by("date")
    for data in datas:
        if len(report) != 0:
            if data.date == report[-1].date:
                report[-1].hour += data.hour
    ...
    context = {
        ...
        "ttopics":tprogress,
        "qtopics":qprogress,
        "progress":progress.first().time_main.all(),
        "report":report,
